...
X = pd.read_csv(BASE_DIR / "Parkinsons_cleaned.csv")
y = pd.read_csv(BASE_DIR / "Parkinsons_status.csv")
groups = pd.read_csv(BASE_DIR / "Parkinsons_groups.csv").values.ravel() 
...
# Train the model one final time on ALL training data before opening the Vault
...
plt.tight_layout()
matrix_output_path = BASE_DIR / "FINAL_Winning_Matrix.png"
plt.savefig(matrix_output_path, dpi=300)
print(f"Figure 1 generated: '{matrix_output_path.name}'")

# PART 4: NOVELTY (FEATURE IMPORTANCE) ----------------------------------------------------------
print("\nExtracting Explainable AI Biomarkers...")

# Dig into the pipeline to get the actual XGBoost model
...
plt.tight_layout()
importance_output_path = BASE_DIR / "Feature_Importance.png"
plt.savefig(importance_output_path, dpi=300)
print(f"Figure 2 generated: '{importance_output_path.name}'")
